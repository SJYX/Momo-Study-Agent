...
3. 需要理解架构、调用链、数据流、入口与依赖关系时：
  - 优先使用结构化搜索/上下文工具；只在已知字符串时用 `rg` / `grep` 精确定位
  - 若用户要求“找出所有出现位置”，先收窄范围，再用精确搜索枚举；架构结论以结构化结果为准
...
- 仅当任务可拆成边界清晰、彼此独立的子问题时，才考虑派发子代理。
- 子代理更适合做只读探索、局部修复、测试补充和单模块分析；共享 contract、配置源、schema 和根入口默认串行处理。
- 派发前先判断是否确有委派价值，并在回复中说明拆分原因与收尾方式。

### 并行开发总控

#### 默认执行模式

- 默认先判断是否适合并行；适合时优先使用当前会话内子代理并行，只有在任务确需独立分支隔离、长期运行或跨终端协作时，才切换到 worktree 模式。
- 当前会话内并行默认持续推进并持续跟踪在途子任务，不因礼貌性确认中断。
- 子代理返回后要统一收尾：汇总改动、检查冲突面、补最终验证，再决定是否需要继续拆分。
- 仅在子代理 `BLOCKED`、需修改共享 contract / schema / 根配置、出现写冲突或依赖冲突、或确需用户验收 / 决策时才打断确认。
