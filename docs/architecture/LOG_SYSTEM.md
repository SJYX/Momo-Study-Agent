...
- `docs/dev/LOGGING.md` 讲接入入口和级别覆盖规则。
- 结构化 JSON、异步写入、节流方法由同一套 logger 管理。
...
- 统计层（可选）：记录函数耗时、模块热度和错误模式；不在本文件展开实现细节。

## 与同步的关系

- 日志系统本身不负责数据同步。
- 主流程退出时会同时触发 `sync_databases()` 和 `sync_hub_databases()`，这是运行时编排的一部分，不属于日志模块职责。
- 前台同步（用户交互触发）通过进度回调显示阶段信息，同时写 logger。
- 后台同步（自动触发线程）仅记录阶段日志（INFO/WARNING），不输出进度条，避免干扰交互界面。
