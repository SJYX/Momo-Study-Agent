...
| `core/config_wizard.py` | 新用户引导向导，含 Token/API Key 联网验证、管理员密码验证、用户信息记录 |
| `core/encryption.py` | Fernet 对称加密工具（API Key 敏感数据加密） |
...
8. **敏感数据加密**：API Key / Token 使用 Fernet 加密，调用 `encryption.encrypt_field()` / `encryption.decrypt_field()`
...
- **已完成**：
  ...
  - 安全密钥体系上线：`.env` 强制要求 `ENCRYPTION_KEY`、`ADMIN_PASSWORD_HASH` 以及 `TURSO_MGMT_TOKEN`
