    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500">{data ? `队列深度: ${data.queue_depth} · 冲突: ${data.conflict_count}` : '加载中...'}</p>
        </div>
      ...
      {data && data.queue_depth > 0 && data.conflicts.length === 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-blue-700 text-sm">
          📤 有 {data.queue_depth} 条待同步记录正在处理中...
        </div>
      )}

      {data && data.conflicts.length === 0 && data.queue_depth === 0 && (
