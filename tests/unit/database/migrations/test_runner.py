...
def test_failure_in_migration_rolls_back_user_version(monkeypatch):
    """V001 之后再追加一个会爆的 V999，确保 V001 的 user_version=1 已落但 V999 不推进版本。"""
    ...
    cur.execute(
        ...
        "raw_full_text TEXT, word_ratings TEXT)"
    )
    conn.commit()
    ...
    # V001 已成功提交，V999 在自己的事务里回滚 → user_version 应停在 1
    cur2 = conn.cursor()
