...
def test_target_version_matches_v007():
...
def test_apply_migrations_to_empty_db():
    """全新空 DB：建骨架 → 跑迁移 → system_config 中 schema_version=7。"""
    conn = _fresh_db()
...
def test_idempotent_second_run_is_noop():
    ...
    assert start1 == 0 and end1 == 7
    # 第二次跑：current >= target，立即返回
    assert start2 == 7 and end2 == 7
...
def test_failure_in_migration_stops_at_last_good_version(monkeypatch):
    """V001~V007 成功后追加一个会爆的 V999，版本应停在 7。"""
    ...
    # V001~V007 已成功提交，V999 回滚 → schema_version 应停在 7
    assert _schema_version(conn) == 7
